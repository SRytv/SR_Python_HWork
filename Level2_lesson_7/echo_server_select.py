...
def mainloop():
    ...
    while True:
        try:
            ...
            #получение списка подключившихся клиентов
            # готовых при этом принять сообщение
            # connected_and_ready_accept = [cli for cli in clients if cli in w]

            for cli in connected_and_ready_accept:
                write_responses(requests, connected_and_ready_accept,
                                clients)  # Выполним отправку ответов клиентам
